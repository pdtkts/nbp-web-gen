...
**⚠️ CRITICAL: GPU 和 CPU 的 ONNX WASM CDN 版本可以不同！**

兩條路徑載入的 WASM 檔案不同，版本要求也不同：

| 路徑 | 檔案 | 載入的 WASM | CDN 版本 |
|------|------|-------------|----------|
| GPU (`useOcrMainThread.js`) | `ort-wasm-simd-threaded.asyncify.wasm` | WebGPU asyncify 專用 | `1.23.2` |
| CPU (`ocr.worker.js`) | `ort-wasm-simd-threaded.wasm` | 標準 WASM | `1.24.1` |

`ort.webgpu.bundle.min.mjs` 只有 ~114KB，**不是真正的 bundle**——WASM 是從 `ort.env.wasm.wasmPaths` 動態載入的。`1.24.1` 的 asyncify WASM 有 WebGPU buffer 生命週期迴歸（`"Buffer used in submit while destroyed"`），所以 GPU 路徑必須用 `1.23.2`。

**DO NOT:**
- 把兩邊的 CDN 版本「對齊」— 它們故意不同
- 升級 GPU 路徑的 WASM 版本，除非先在 WebGPU 模式下實測確認無 buffer 錯誤
